...
object ConferenceProtocol extends AggregateProtocol[ConferenceState, ConferenceState#ID] {
  ...
  final case class VerifiedCreateConference private[conference] ( conference: ConferenceInfo )
      extends ConferenceMessage

  final case class SlugTaken private[conference] ( targetId: ConferenceModule.TID, slug: String )
      extends ConferenceMessage

  case class CreateConference(
    override val targetId: CreateConference#TID,
    conference: ConferenceInfo
  ) extends ConferenceCommand
  case class UpdateConference(
    override val targetId: UpdateConference#TID,
    conference: ConferenceInfo
  ) extends ConferenceCommand

  case class CreateSeat( override val targetId: CreateSeat#TID, seat: SeatType )
      extends ConferenceCommand
  case class UpdateSeat( override val targetId: UpdateSeat#TID, seat: SeatType )
      extends ConferenceCommand
  case class DeleteSeat( override val targetId: DeleteSeat#TID, seatId: SeatType.TID )
      extends ConferenceCommand
  ...
  case class ConferenceCreated(
    override val sourceId: ConferenceCreated#TID,
    conference: ConferenceInfo
  ) extends ConferenceEvent
  //Conference/Conference.Contracts/ConferenceUpdated.cs
  case class ConferenceUpdated(
    override val sourceId: ConferenceUpdated#TID,
    conference: ConferenceInfo
  ) extends ConferenceEvent
  //Conference/Conference.Contracts/ConferencePublished.cs
  case class ConferencePublished( override val sourceId: ConferencePublished#TID )
      extends ConferenceEvent
  //Conference/Conference.Contracts/ConferenceUnpublished.cs
  case class ConferenceUnpublished( override val sourceId: ConferenceUnpublished#TID )
      extends ConferenceEvent
  //Conference/Conference.Contracts/SeatCreated.cs
  case class SeatCreated( override val sourceId: SeatCreated#TID, seatType: SeatType )
      extends ConferenceEvent
  //Conference/Conference.Contracts/SeatUpdated.cs
  case class SeatUpdated( override val sourceId: SeatUpdated#TID, seatType: SeatType )
      extends ConferenceEvent
  case class SeatDeleted( override val sourceId: SeatDeleted#TID, seatTypeId: SeatType.TID )
      extends ConferenceEvent
}
...
object ConferenceModule extends AggregateRootModule[ConferenceState, ConferenceState#ID] { module =>
  ...
  class Repository( model: DomainModel )
      extends EnvelopingAggregateRootRepository( model, ConferenceType )
  ...
  class Conference(
  ...
  ) extends AggregateRoot[ConferenceState, ShortUUID]
    ...
    override val acceptance: Acceptance = {
      ...
      case ( SeatCreated( _, seatType ), state ) =>
        ConferenceState.seatsLens.set( state )( state.seats + seatType )
    ...
    val quiescent: Receive = LoggingReceive {
      case CreateConference( id, conference ) if !conference.slug.isEmpty => {
        ...
        val askForSlugStatus = conferenceContext
          .askretry(
    ...
    def draft: Receive = LoggingReceive {
      case UpdateConference( _, conference ) =>
        persist( ConferenceCreated( state.id, conference ) ) { acceptAndPublish }
      ...
      case DeleteSeat( _, seatId ) =>
        persist( SeatDeleted( state.id, seatId ) ) { acceptAndPublish }
      case Publish => {
    ...
    def published: Receive = LoggingReceive {
      case Unpublish =>
        persist( ConferenceUnpublished( state.id ) ) { event =>
          acceptAndPublish( event )
          context.become( around( draft ) )
        }
    }
  ...
  final case class UnspecifiedConferenceContextError private[conference] ( expectedKey: Symbol )
      extends IllegalArgumentException(
        s"ConferenceContext actor ref required at initialization property [$expectedKey]"
      )
      with ContosoError
}
