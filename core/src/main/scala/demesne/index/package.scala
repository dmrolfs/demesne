 ...
 *
 * [[Index]] clients can use the instance to easily look up [[AggregateRoot]] identifiers based on logical keys or in bulk.
 *
 ...
 * [[IndexAggregate]]. The IndexAggregate records the logical key to identifier mapping and published a Recorded event, which
 * is picked up by local [[Index]]s, such as an in-memory [[demesne.index.local.IndexLocalAgent]] or in the future a
 * index backed by some other form of cache (perhaps Redis).
 *
 * [[AggregateRoot]]s override the publish() operation to publish events to the Index Index. [[IndexRelay]] actors listen
 * for events published either via the IndexBus (default) or the ContextChannel subscription for a specific class. Aggregates
 * can publish to additional channels by adding to the publish chain.
 *
 * [[Index]] subscriptions are registered by overriding the [[AggregateRootType]].indexes() operation. Each
